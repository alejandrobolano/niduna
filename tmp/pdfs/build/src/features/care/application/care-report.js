...
exports.careReportColumns = void 0;
exports.createCareReportHtml = createCareReportHtml;
exports.createCareReportFileName = createCareReportFileName;
const care_event_description_1 = require("./care-event-description");
const care_snapshot_1 = require("./care-snapshot");
exports.careReportColumns = [
...
function summarizeEvents(events) {
    ...
    const feedingIntervals = feedings.slice(1).map((event, index) => Math.max(0, Math.round((Date.parse(event.occurredAt) - Date.parse(feedings[index].occurredAt)) / 60_000)));
    ...
    return {
        ...
        feeding: {
            averageIntervalMinutes: feedingIntervals.length > 0
                ? Math.round(feedingIntervals.reduce((total, interval) => total + interval, 0) /
                    feedingIntervals.length)
                : undefined,
            count: feedings.length,
            totalAmountMilliliters: feedings.reduce((total, event) => total + (event.amountMilliliters ?? 0), 0),
        },
        noteCount: events.filter((event) => event.type === 'note').length,
        sleepMinutes: events.reduce((total, event) => event.type === 'sleep' && event.endedAt
            ? total + (0, care_snapshot_1.getDurationMinutes)(event.occurredAt, event.endedAt)
            : total, 0),
    };
}
...
function formatPeriod(events) {
    ...
    const format = (value) => new Intl.DateTimeFormat('es-ES', {
        dateStyle: 'medium',
    ...
    <tbody>
      ${events.map((event) => {
        const values = {
            ...
            detail: (0, care_event_description_1.describeCareEvent)(event),
            type: care_event_description_1.careEventLabels[event.type],
        };
...
function createCareReportHtml({ babyName, columns, contacts, events, familyName, filterLabel, generatedAt = new Date(), }) {
    ...
    <body>
      ...
      <main>
        ...
        <section class="scope">
          ...
          <div><span>COLUMNAS</span><strong>${columns.map((column) => exports.careReportColumns.find((item) => item.value === column)?.label).filter(Boolean).join(', ')}</strong></div>
        </section>
...
function createCareReportFileName(babyName, date = new Date()) {
